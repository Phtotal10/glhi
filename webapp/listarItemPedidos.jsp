			...
			<tbody>
				<c:forEach items="${listaItemPedidos}" var="itemPedido">
					<tr>
						...
						<td><a
	href="ItemPedidoController?acao=atualizar&id_item_pedido=<c:out value="${itemPedido.id}"/>&quantidade=<c:out value="${itemPedido.quantidade}"/>&id_pedido=<c:out
                        value="${itemPedido.pedido.id}"/>&id_produto=<c:out
value="${itemPedido.produto.id}"/>">
								Alterar </a></td>
						<td><a
							href="ItemPedidoController?acao=remover&id_item_pedido=<c:out value="${itemPedido.id}"/>"
			...
			<tfoot>
				<tr>
					<td colspan='7'><a
						href="ItemPedidoController?acao=criar&id_pedido=<c:out value="${pedido.id}"/> ">Incluir
							Novo Item Pedido </a></td>
				</tr>
			</tfoot>
		</table>
	</form>
